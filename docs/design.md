...
- **Per-VM lock serializes `run` and `cleanup`:** the same crash-then-orphan scenario above means a `cleanup` invoked at the wrong moment could race a `run` that's still mid-choreography on the same VM — deleting a snapshot `run` still expects to find waiting for it. `internal/backup.Lock` closes that gap: an exclusive, non-blocking, per-VM advisory lock at `<destination>/.snapback-locks/<vm-name>.lock`, held by `run` for its entire choreography and taken by `cleanup` before it deletes anything, so the two commands can never touch the same VM's snapshots at once. They don't behave symmetrically when the lock is already held, though, and that's deliberate: `cleanup` defers to an in-progress `run` and reports rather than erroring (there's always a later chance to clean up), while `run` fails outright (silently skipping a scheduled backup is worse than a loud failure). The lock is backed by `flock(2)` rather than a lockfile-plus-PID scheme specifically because of the crash case this whole mechanism exists to handle: the kernel releases a `flock` automatically when the holding process's file descriptor closes, dead or alive, so a `run` that crashes — leaving behind exactly the orphaned snapshot described above — can never also leave behind a stale lock that permanently blocks `cleanup`.
...
- **Merging a snapshot on a powered-off VM is not reliably synchronous (confirmed real incident, 2026-09-08):** `run` against a stopped VM took a snapshot and merged it back with `vmcli` reporting success both times, but the on-disk consolidation silently didn't apply — the VM couldn't power on afterward, `vmware-vdiskmanager -e` reported "needs repair" on the affected `.vmdk`. `vmcli`'s exit code alone is not proof the merge landed. Fixed by verifying disk-chain consistency (`vm.Controller.CheckDiskConsistency`, backed by `vmware-vdiskmanager -e`) both before taking a new snapshot and again right after the merge — but only while the VM isn't running: its disk files are held open by the live `vmware-vmx` process, so the check can't run at all in that case (fails on lock contention, not a real verdict), and doesn't need to, since a running VM's chain is already known-good by the fact that it's running.
- **Fusion lock directories leak into archives and break restore's disk-consistency check (confirmed real incident, 2026-09-11):** restoring a real archive failed with "Disk chain is not consistent: The specified directory is not empty (0x4e26)". Fusion creates a `<file>.lck` directory (holding a per-process `M<pid>.lck` file) next to any `.vmx`/`.vmdk` currently open, and step 6's copy reads the live bundle while the VM is still running, so these lock directories were always present at copy time and got archived verbatim. Restoring reproduced them, and `vmware-vdiskmanager -e` mistook the stale, restored lock directory for one another process already had the disk open under, failing consistency over nothing. `createArchive` now skips any `*.lck` directory when taring the staged bundle; `extractArchive` also drops any `*.lck` tar entry (directory-only match — a plain file whose own name happens to end in `.lck` is left alone) so archives made before the fix still restore cleanly.
- **Restore always trips Fusion's "moved or copied" prompt.** A restored bundle lands at a brand-new path, so Fusion's path-hash-based UUID check never matches and it prompts the first time the bundle is opened. The only correct answer is "I Copied It" — the untouched source keeps its original identity, so answering "I Moved It" would let two VMs claim the same UUID/MAC if both are ever powered on. `Restore` writes `uuid.action = "create"` into the restored `.vmx` so Fusion applies that answer silently instead of asking.
